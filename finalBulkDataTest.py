...
pathToDropboxFiles = "C:\Users\molly\Documents\BaxterLab\CRIS\AutomatedCRISDataTesting\SFF_DropboxFiles"
pathToCRISFiles = "C:\Users\molly\Documents\BaxterLab\CRIS\AutomatedCRISDataTesting\SFF_CRISFiles"
reportFile = "C:\Users\molly\Documents\BaxterLab\CRIS\AutomatedCRISDataTesting\Report_1027.txt"
...
reportObj['sameNumCols']['noRowValuesChanged'] = []
	...
	if "CRIS_"+file in CRISFiles:
		...
		with open(os.path.join(pathToDropboxFiles, file), 'r') as Dfile, open(os.path.join(pathToCRISFiles, ("CRIS_"+file)), 'r') as Cfile:
			...
			if not colInFileDOnly and not colInFileCOnly:
				subprocess.call(['csvdiff', '--style', 'pretty','--output', 'diff1027.json', 'fullnum', os.path.join(pathToDropboxFiles, file), os.path.join(pathToCRISFiles, ("CRIS_"+file))], shell=True)
				diffData = json.load(open('diff.json'))

				# HERE - do a check to see if there's anything in the 'changed' list... report if its empty
				if not diffData['changed']:
					reportObj['sameNumCols']['noRowValuesChanged'].append(file)
				else:
					changedCols = []
